...
module.exports = {
  comment: (content, commentKey, handleResponse) => {
    if (!handleResponse) {
      handleResponse = r => console.log("comment", r.status)
    }
  ...
  feedback: (content, programID, handleResponse) => {
    if (!handleResponse) {
      handleResponse = r => console.log("feedback", r.status)
    }
  ...
  del_feedback: (kaencrypted, programID, handleResponse) => {
    if (!handleResponse) {
      handleResponse = r => console.log("del_feedback", r.status)
    }
  ...
  updateProgram: (id, newCode, newTitle, newWidth, newHeight, handleResponse) => {
    if (!handleResponse) {
